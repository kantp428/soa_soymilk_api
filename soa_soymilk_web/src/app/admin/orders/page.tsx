                  ...
                  {isLoading ? (
                    ...
                    <tr key={order.order_id || `order-${idx}`} className="border-b transition-colors hover:bg-zinc-50/50">
                      ...
                      <td className="p-4 align-middle">
                        {order.order_time? new Date(order.order_time.replace(' ', 'T')).toLocaleString('th-TH') : 'N/A'}
                      </td>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {orderDetail.items && orderDetail.items.length > 0 ? (
                        ...
                        <div>
                          ...
                          {item.addons && item.addons.length > 0 && (() => {
                            const grouped = item.addons.reduce((acc, addon) => {
                              if (!acc[addon.addon_name]) {
                                acc[addon.addon_name] = { count: 0, price: addon.price };
                              }
                              acc[addon.addon_name].count += 1;
                              return acc;
                            }, {} as Record<string, { count: number, price: number }>);

                            return (
                              <div className="mt-2 space-y-1 pl-3 border-l-2 border-zinc-200">
                                {Object.entries(grouped).map(([name, data]) => (
                                  <div key={name} className="flex justify-between items-center text-[11px] text-zinc-500">
                                    <span>↳ {name} <span className="text-zinc-400 text-[10px] ml-1">x{data.count}</span></span>
                   ...
                   <p className="text-sm text-zinc-500 text-center py-8 border-2 border-dashed border-zinc-100 rounded-xl">
                      ไม่มีข้อมูลสินค้า (อาจบันทึกไม่สมบูรณ์ หรือ API ไม่ได้ส่งกลับมา)
                   </p>
                 )}
                </div>
              </div>
